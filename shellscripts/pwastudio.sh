...
cat > package.json << EOF
{
  ...
  "scripts": {
    "build-venia": "yarn workspace @magento/venia-concept run build",
    "build-$scriptName": "yarn workspace @$companyName/$templateName run build",
    ...
    "stage:$scriptName": "yarn workspace @$companyName/$templateName run start; cd - >/dev/null",
    "stats:venia": "yarn workspace @magento/venia-concept run build:analyze && yarn workspace @magento/venia-concept run stats",
    "stats:$scriptName": "yarn workspace @$companyName/$templateName run build:analyze && yarn workspace @$companyName/$templateName run stats",
    ...
    "validate-queries-$scriptName": "yarn workspace @$companyName/$templateName run validate-queries",
    ...
    "watch:$scriptName": "yarn workspace @$companyName/$templateName run watch; cd - >/dev/null"
},
  "devDependencies": {
...
echo -e "\e[1;32m"
echo "#########################"
echo "# Step 8.1: Creating .env inside ur theme #"
echo "#########################"
echo -e "\e[00m"
...
#
#   Generated by @magento/pwa-buildpack v3.0.0 on 2019-07-22T18:42:52.085Z.
#
################################################################################
...
#
#   Specify a Braintree API token token to direct the Venia storefront to 
#   communicate with your Braintree instance. You can find this value in 
#   Braintree's Control Panel under Settings > API Keys > Tokenization Keys.
#   - Default when not set: sandbox_8yrzsvtm_s2bg8fs563crhqzk
